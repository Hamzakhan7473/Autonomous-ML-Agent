...
class ModelEvaluator:
    ...
    def evaluate(
        self, model, X: pd.DataFrame, y: pd.Series, cv_folds: int | None = None
    ) -> dict[str, Any]:
    ...
    def _cross_validate(
        self, model, X: pd.DataFrame, y: pd.Series, cv_folds: int
    ) -> dict[str, Any]:
        ...
        if model.config.is_classification and self.config.stratified_cv:
            cv = StratifiedKFold(
                n_splits=cv_folds, shuffle=True, random_state=self.config.random_state
            )
        else:
            cv = KFold(
                n_splits=cv_folds, shuffle=True, random_state=self.config.random_state
    ...
    def compare_models(
        self, results: list[dict[str, Any]], primary_metric: str | None = None
    ) -> pd.DataFrame:
    ...
    def get_best_model(
        ...
        for result in results:
            ...
            for key in [
                f"{primary_metric}_cv_mean",
                f"{primary_metric}_test",
                primary_metric,
            ]:
                if key in cv_results:
                    score = (
                        cv_results[key]["mean"]
                        if isinstance(cv_results[key], dict)
                        else cv_results[key]
                    )
                    break
            else:
...
def evaluate_model_performance(
    model, X: pd.DataFrame, y: pd.Series, config: EvaluationConfig | None = None
) -> dict[str, Any]:
...
def compare_models(
    results: list[dict[str, Any]], primary_metric: str | None = None
) -> pd.DataFrame:

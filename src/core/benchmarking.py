...
from .model_zoo import ModelZoo, ModelConfig
...
class ModelBenchmark:
    ...
    def benchmark_single_model(self, 
                              ...
                              task_type: Optional[str] = None) -> ModelResult:
        """
        ...
        try:
            ...
            if model_config.is_classification != (task_type == 'classification'):
                raise ValueError(f"Model {model_name} is for {'classification' if model_config.is_classification else 'regression'}, "
        ...
        for metric in scoring_metrics:
            try:
                scores = cross_val_score(
                    model.model, X, y, cv=cv, scoring=metric, 
    ...
    def _log_to_mlflow(self, result: ModelResult, X: pd.DataFrame, y: pd.Series):
        ...
        try:
            ...
            self.mlflow_tracker.log_data_info(
                ...
                class_distribution=y.value_counts().to_dict() if result.task_type == 'classification' else None
            )
    ...
    def benchmark_multiple_models(self, 
                                ...
                                hyperparameters: Optional[Dict[str, Dict[str, Any]]] = None) -> List[ModelResult]:
        """
    ...
    def benchmark_all_models(self, 
                            ...
                            y: pd.Series,
                            task_type: Optional[str] = None,
                            max_models: Optional[int] = None) -> List[ModelResult]:
        """
    ...
    def _create_performance_plots(self, results: List[ModelResult]) -> List[str]:
        ...
        importance_results = [r for r in successful_results if r.feature_importance is not None]
        if importance_results:
            fig, ax = plt.subplots(figsize=(12, 8))
            
            # Get feature names (assuming all models have same features)
            feature_names = [f"Feature_{i}" for i in range(len(importance_results[0].feature_importance))]
            
            # Plot feature importance for top models
            top_models = sorted(importance_results, 
                             key=lambda x: x.test_metrics.get('accuracy' if x.task_type == 'classification' else 'r2_score', 0),
                             reverse=True)[:3]
            
            for i, result in enumerate(top_models):
                ax.plot(feature_names, result.feature_importance, 
                       marker='o', label=f"{result.model_name}", alpha=0.7)
    ...
    def export_results(self, 
        ...
        for result in results:
            ...
            if result_dict['feature_importance'] is not None:
                result_dict['feature_importance'] = result_dict['feature_importance'].tolist()
...
def run_benchmark_experiment(dataset_path: str,
                           target_column: str,
                           output_dir: str = "benchmarks",
                           max_models: Optional[int] = None,
                           use_mlflow: bool = True) -> str:
    """
    ...
    mlflow_tracker = None
    if use_mlflow:
        mlflow_tracker = MLflowTracker(experiment_name=f"benchmark_{Path(dataset_path).stem}")
    ...
    # Export results
    results_path = Path(output_dir) / f"benchmark_results_{Path(dataset_path).stem}.json"

...
async def get_task_status(task_id: str):
    ...
    if task_info["status"] == "running":
        ...
        return TaskStatus(
            task_id=task_id, status="unknown", progress=0.0, message="Unknown status"
        )
...
async def make_prediction(request: PredictionRequest):
    ...
    try:
        ...
        confidence = None
        if (
            probabilities is not None and len(probabilities[0]) == 2
...
async def get_model_info(task_id: str):
    ...
    try:
        ...
        return {
            ...
            "model_info": summary,
            "feature_importance": (
                agent.get_feature_importance().tolist()
                if agent.get_feature_importance() is not None
                else None
            ),
            "preprocessing_config": (
                agent.preprocessor.config.__dict__ if agent.preprocessor else None
            ),
        }

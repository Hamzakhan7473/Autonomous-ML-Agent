...
from core.mlflow_tracker import MLflowTracker, setup_mlflow_experiment
from core.benchmarking import ModelBenchmark, BenchmarkConfig, run_benchmark_experiment
...
def create_sample_dataset(n_samples: int = 1000, n_features: int = 10, task_type: str = "classification") -> str:
...
def demo_mlflow_tracking():
    ...
    tracker.log_data_info(
        ...
        class_distribution=y.value_counts().to_dict() if len(y.unique()) <= 20 else None
    )
...
def demo_model_benchmarking():
    ...
    benchmark = ModelBenchmark(
        config=BenchmarkConfig(cv_folds=3, verbose=True),
    ...
    available_models = zoo.list_models(task_type == 'classification')[:3]  # Top 3 for demo
    ...
    report_path = benchmark.create_comparison_report(results, "./benchmark_demo/manual_benchmark.html")

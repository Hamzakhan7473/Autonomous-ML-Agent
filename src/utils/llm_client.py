    ...
    def generate_response(self, prompt: str, **kwargs) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert machine learning engineer."},
...
class LLMClient:
    ...
    def __init__(self, primary_provider: str = "openai", **kwargs):
    ...
    def _initialize_clients(self, **kwargs):
        ...
        if not self.clients:
            raise ValueError("No LLM clients available. Please check your API keys and dependencies.")
    ...
    def generate_response(self, prompt: str, provider: Optional[str] = None, **kwargs) -> str:
        ...
        try:
            response = self.clients[selected_provider].generate_response(prompt, **kwargs)
            ...
            for fallback_provider in self.clients:
                if fallback_provider != selected_provider:
                    try:
                        response = self.clients[fallback_provider].generate_response(prompt, **kwargs)
    ...
    def generate_structured_response(self, prompt: str, schema: Dict[str, Any], 
        ...
        try:
            response = self.clients[selected_provider].generate_structured_response(prompt, schema, **kwargs)
            ...
            for fallback_provider in self.clients:
                if fallback_provider != selected_provider:
                    try:
                        response = self.clients[fallback_provider].generate_structured_response(prompt, schema, **kwargs)
                        logger.info(f"Used fallback provider: {fallback_provider}")
                        return response
                    except Exception as fallback_error:
                        logger.error(f"Fallback {fallback_provider} also failed: {fallback_error}")
    ...
    def test_connection(self, provider: Optional[str] = None) -> bool:
        ...
        for prov in providers_to_test:
            try:
                response = self.clients[prov].generate_response(test_prompt, max_tokens=10)

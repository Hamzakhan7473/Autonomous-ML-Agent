...
class ModelInterpreter:
    ...
    def get_feature_importance(self, X: np.ndarray, y: np.ndarray = None) -> Dict[str, float]:
        """
    ...
    def _calculate_permutation_importance(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            logger.warning("Could not calculate permutation importance, using random values")
    ...
    def get_global_explanation(self, X: np.ndarray, y: np.ndarray = None) -> Dict[str, Any]:
        """
    ...
    def _get_top_features(self, feature_importance: Dict[str, float], top_k: int = 10) -> List[Tuple[str, float]]:
    ...
    def generate_insights(self, X: np.ndarray, y: np.ndarray = None) -> str:
        ...
        insights = []
        insights.append(f"The model is a {type(self.model).__name__} trained on {X.shape[0]} samples with {X.shape[1]} features.")
    ...
    def create_explainer(self, X: np.ndarray):
        ...
        try:
            ...
            if hasattr(self.model, 'predict_proba'):
                ...
                self.explainer = shap.TreeExplainer(self.model) if hasattr(self.model, 'feature_importances_') else shap.KernelExplainer(self.model.predict_proba, X[:100])
            else:
                # Regression model
                self.explainer = shap.TreeExplainer(self.model) if hasattr(self.model, 'feature_importances_') else shap.KernelExplainer(self.model.predict, X[:100])
...
class LimeExplainer:
    ...
    def explain_sample(self, X: np.ndarray, sample_idx: int = 0, num_features: int = 10) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                'prediction': float(self.model.predict(X[sample_idx:sample_idx+1])[0])
            }
        except Exception as e:
...
class ModelInsightsGenerator:
    ...
    def generate_comprehensive_insights(self, X: np.ndarray, y: np.ndarray = None) -> str:
        """
        Generate comprehensive model insights.
        ...
        if feature_importance:
            insights.append("\n🔍 FEATURE IMPORTANCE")
            insights.append("-" * 30)
            top_features = self.interpreter._get_top_features(feature_importance, top_k=10)
            for i, (feature, importance) in enumerate(top_features, 1):
        ...
        if y is not None:
            ...
            if hasattr(self.model, 'predict_proba'):
                ...
                insights.append(f"Class Distribution: {dict(zip(unique_classes, counts))}")
            else:
                # Regression
        ...
        if feature_importance:
            ...
            if top_feature:
                insights.append(f"• Focus on {top_feature} as it's the most important feature")
        ...
        if hasattr(self.model, 'max_depth') and self.model.max_depth is not None and self.model.max_depth > 10:

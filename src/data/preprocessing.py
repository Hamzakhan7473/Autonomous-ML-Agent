...
class DataPreprocessor:
    ...
    def create_preprocessing_pipeline(self, df: pd.DataFrame) -> Pipeline:
        ...
        if self.numerical_features:
            ...
            transformers.append(('numerical', numerical_transformer, self.numerical_features))
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        if self.categorical_features:
            categorical_transformer = self.preprocessing_pipeline.named_transformers_['categorical']
        ...
        return feature_names.tolist() if hasattr(feature_names, 'tolist') else feature_names
...
def handle_missing_values(df: pd.DataFrame, strategy: str = 'auto') -> pd.DataFrame:
    ...
    if strategy == 'auto':
        ...
        if missing_ratio.max() > 0.5:
            ...
            logger.info(f"Dropped columns with >50% missing values: {columns_to_drop.tolist()}")

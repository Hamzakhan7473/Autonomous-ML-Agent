...
class DataIngestion:
    ...
    def validate_data(self, df: pd.DataFrame, target_column: str) -> DatasetInfo:
        ...
        logger.info(
            f"Dataset validation completed: {info.shape[0]} rows, {info.shape[1]} columns"
        )
        return info
    ...
    def detect_data_issues(self, df: pd.DataFrame, target_column: str) -> list[str]:
        ...
        if not high_missing_cols.empty:
            issues.append(
                f"High missing values (>50%) in columns: {list(high_missing_cols.index)}"
            )
        ...
        if high_cardinality_cols:
            issues.append(
                f"High cardinality categorical features: {high_cardinality_cols}"
            )

        return issues

    def suggest_preprocessing_steps(
        self, df: pd.DataFrame, target_column: str
    ) -> list[str]:
        """

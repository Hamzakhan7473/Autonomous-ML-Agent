...
class MetaFeatureExtractor:
    ...
    def _extract_basic_features(self, df: pd.DataFrame):
        ...
        self.meta_features.update({
            ...
            'categorical_features_ratio': len(features.select_dtypes(include=['object', 'category']).columns) / len(features.columns),
            'numerical_features_ratio': len(features.select_dtypes(include=[np.number]).columns) / len(features.columns)
        })
    ...
    def _extract_information_theoretic_features(self, df: pd.DataFrame):
        ...
        if len(numerical_features.columns) > 0:
            try:
                if target.dtype == 'object':
                    mi_scores = mutual_info_classif(numerical_features, target, random_state=42)
                else:
                    mi_scores = mutual_info_regression(numerical_features, target, random_state=42)
    ...
    def _extract_feature_based_features(self, df: pd.DataFrame):
        ...
        self.meta_features['zero_variance_features_ratio'] = len(zero_var_features) / len(features.columns)
        
        # Number of features with low variance
        low_var_features = features.columns[features.var() < 0.01]
        self.meta_features['low_variance_features_ratio'] = len(low_var_features) / len(features.columns)
        
        # Feature sparsity
        numerical_features = features.select_dtypes(include=[np.number])
        if len(numerical_features.columns) > 0:
            sparsity = (numerical_features == 0).sum().sum() / (len(numerical_features) * len(numerical_features.columns))
    ...
    def _calculate_fisher_score(self, features: pd.DataFrame, target: pd.Series) -> float:
    ...
    def _analyze_target(self, df: pd.DataFrame) -> Dict:
        ...
        if target.dtype == 'object':
            ...
            analysis.update({
                ...
                'most_common_class': target.mode()[0] if len(target.mode()) > 0 else None
            })
        else:
            # Regression
            analysis.update({
        ...
        if len(categorical_features.columns) > 0:
            ...
            for col in categorical_features.columns:
                cat_stats[col] = {
                    ...
                    'most_common': categorical_features[col].mode()[0] if len(categorical_features[col].mode()) > 0 else None,
...
class MetaLearningFeatures:
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            'num_instances', 'num_features', 'num_classes', 'missing_values_ratio',
            'categorical_features_ratio', 'numerical_features_ratio',

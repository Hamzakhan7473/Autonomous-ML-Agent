...
try:
    ...
    warnings.warn(
        "XGBoost not available. Install with: pip install xgboost", stacklevel=2
    )
...
try:
    ...
    warnings.warn(
        "LightGBM not available. Install with: pip install lightgbm", stacklevel=2
    )
...
try:
    ...
    warnings.warn(
        "CatBoost not available. Install with: pip install catboost", stacklevel=2
    )
...
class ModelZoo:
    ...
    def get_model(self, name: str, is_classification: bool) -> BaseModel:
        ...
        if config.is_classification != is_classification:
            raise ValueError(
                f"Model '{name}' is for {'classification' if config.is_classification else 'regression'}, "
    ...
    def get_recommended_models(
        self, is_classification: bool, n_samples: int, n_features: int
    ) -> list[str]:
        ...
        model_priorities = [
            (name, self.models[name].priority) for name in available_models
        ]
        ...
        if n_samples < 1000:
            recommended = [
                name
                for name, _ in model_priorities
                if name
                in ["logistic_regression", "linear_regression", "naive_bayes", "lda"]
            ]
        else:

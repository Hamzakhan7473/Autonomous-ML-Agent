...
class ModelEvaluator:
    ...
    def evaluate_model(
        ...
        if hasattr(model, "predict_proba"):
            ...
            results = {"task_type": "regression", "metrics": metrics}
...
class ModelComparison:
    ...
    def add_model(
        self, name: str, model: BaseEstimator, X_test: np.ndarray, y_test: np.ndarray
    ):
...
class StatisticalSignificance:
    ...
    @staticmethod
    def mcnemar_test(
        model1_pred: np.ndarray, model2_pred: np.ndarray, y_true: np.ndarray
    ) -> dict[str, float]:
        ...
        if only1_correct + only2_correct == 0:
            ...
            chi2_stat = (abs(only1_correct - only2_correct) - 1) ** 2 / (

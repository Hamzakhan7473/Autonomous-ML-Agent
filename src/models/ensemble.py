...
class EnsembleBuilder:
    ...
    def build_ensemble(self, X: np.ndarray, y: np.ndarray) -> BaseEstimator:
        ...
        logger.info(
            f"Built {self.ensemble_method} ensemble with {len(self.base_models)} base models"
        )
...
class AutoEnsembleBuilder:
        ...
        for method in self.ensemble_methods:
            try:
                ...
                scores = cross_val_score(
                    ensemble, X, y, cv=self.cv, scoring=self.scoring, n_jobs=-1
                )
                mean_score = scores.mean()

                logger.info(
                    f"{method} ensemble score: {mean_score:.4f} (+/- {scores.std() * 2:.4f})"
                )
    ...
    def get_best_ensemble(self) -> BaseEstimator | None:
        """Get the best ensemble after optimization."""
        return self.best_ensemble

    def get_best_method(self) -> str | None:
...
class WeightedEnsemble:
    ...
    def __init__(
        self, models: dict[str, BaseEstimator], weight_optimization: str = "uniform"
    ):
        """
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if self.weight_optimization == "uniform":
            ...
            raise ValueError(
                f"Unknown weight optimization method: {self.weight_optimization}"
            )
    ...
    def _optimize_weights(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        def objective(weights):
            ...
            for i, (_name, pred) in enumerate(predictions.items()):
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        for i, (_name, proba) in enumerate(proba_predictions.items()):
...
def create_ensemble(
    models: dict[str, BaseEstimator], method: str = "voting", **kwargs
) -> BaseEstimator:
    """

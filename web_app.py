...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🤖 Autonomous Machine Learning Agent</h1>', unsafe_allow_html=True)
    st.markdown("""
    ...
    with st.sidebar:
        ...
        if not use_mock_llm:
            ...
            llm_model = st.text_input(
                "LLM Model",
                value="gpt-4" if llm_provider == "openai" else "claude-3-sonnet-20240229"
            )
...
def display_results(results):
    ...
    """.format(insights=results.model_insights), unsafe_allow_html=True)

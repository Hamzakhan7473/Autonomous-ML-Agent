...
class WeightedBlender(BaseBlender):
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        for proba, weight in zip(probabilities, self.weights, strict=False):
...
class StackingBlender(BaseBlender):
    ...
    def fit(
        ...
        self.is_fitted = True
        logger.info(
            f"Stacking blender fitted with meta-model: {self.meta_model.__class__.__name__}"
        )
        return self
    ...
    def _create_meta_features(self, X: pd.DataFrame, y: pd.Series) -> np.ndarray:
        ...
        for _fold, (train_idx, val_idx) in enumerate(cv.split(X, y)):
...
class EnsembleBlender:
    ...
    def __init__(self, config: BlendingConfig | None = None):
    ...
    def create_blender(self, method: str | None = None) -> BaseBlender:
...
def create_ensemble(
    models: list[Any], X: pd.DataFrame, y: pd.Series, method: str = "weighted", **kwargs
) -> BaseBlender:

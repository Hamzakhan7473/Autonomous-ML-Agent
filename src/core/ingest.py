...
class DataIngester:
    ...
    def validate_data(self, df: pd.DataFrame, target_column: str) -> Tuple[bool, List[str]]:
        ...
        if target_missing > len(df) * 0.5:
            issues.append(f"Target column has {target_missing} missing values ({target_missing/len(df)*100:.1f}%)")
        
        # Check for duplicate rows
        duplicates = df.duplicated().sum()
        if duplicates > len(df) * 0.1:
            issues.append(f"Dataset has {duplicates} duplicate rows ({duplicates/len(df)*100:.1f}%)")
    ...
    def get_data_summary(self, df: pd.DataFrame, target_column: str) -> Dict[str, Any]:
        ...
        numerical_cols = [col for col, dtype in schema.feature_types.items() 
                         if dtype == 'numerical']
        numerical_summary = {}
        ...
        # Categorical summary
        categorical_cols = [col for col, dtype in schema.feature_types.items() 
        ...
        return {
            ...
            'data_quality': {
                ...
                'high_missing_features': [col for col in df.columns 
                                        if col != target_column and df[col].isna().sum() > len(df) * 0.5]
            }
        }


def analyze_data(file_path: str, target_column: str, **kwargs) -> Tuple[pd.DataFrame, DatasetSchema, Dict[str, Any]]:

...
class FeatureEngineer:
    ...
    def create_interaction_features(self, df: pd.DataFrame, max_features: int = 10) -> pd.DataFrame:
...
class DataPreprocessor:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
            ...
            if col in X_processed.columns and col in self.encoders:
                ...
                if isinstance(encoder, OneHotEncoder):
                    ...
                    encoded_df = pd.DataFrame(
                        encoded,
                        columns=[f"{col}_{cat}" for cat in encoder.categories_[0][1:]],  # Skip first category
        ...
        if self.scaler is not None and len(self.numerical_columns) > 0:
            ...
            existing_numerical = [col for col in self.numerical_columns if col in X_processed.columns]
            if existing_numerical:
                X_processed[existing_numerical] = self.scaler.transform(X_processed[existing_numerical])

...
try:
    ...
    warnings.warn(
        "Optuna not available. Install with: pip install optuna", stacklevel=2
    )
...
class RandomSearchOptimizer(BaseOptimizer):
    ...
    def _cross_validate(self, model, X: pd.DataFrame, y: pd.Series) -> list[float]:
        ...
        scores = cross_val_score(
            model.model, X, y, cv=cv, scoring=scoring, n_jobs=self.config.n_jobs
        )
...
class OptunaOptimizer(BaseOptimizer):
    ...
    def optimize(
        ...
        # Optimize
        self.study.optimize(
            objective, n_trials=self.config.n_trials, timeout=self.config.timeout
        )
...
class HyperparameterOptimizer:
    ...
    def optimize(
        ...
        try:
            best_model, best_score, best_params = self.optimizer.optimize(
                self.model, X, y
            )
...
def optimize_model(
    model, X: pd.DataFrame, y: pd.Series, **kwargs
) -> tuple[Any, float, dict[str, Any]]:
